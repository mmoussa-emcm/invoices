...
const SS = process.env.SS; // set a secret token for admin panel access
  ...
  res.send(`
    <html>
      ...
      <body>
        ...
        <form method="POST" action="/admin/config?token=${SS}">
          <label>
...
app.post("/admin/config", (req, res) => {
  ...
  saveConfig();
  res.send(
    'Settings updated. <a href="/admin/config?token=' + SS + '">Back</a>'
  );
});
  ...
  res.send(`
    ...
    <html>
        ...
        <script>
          document.addEventListener('DOMContentLoaded', () => {
            ...
            AppBridge.NavigationMenu.create(app, {
              items: [
                { label: 'Invoice Settings', destination: '/admin/config?token=${SS}' }
              ],
            });
          });
        </script>
      ...
      <body style="margin:0;padding:0;">
        <iframe src="/admin/config?token=${SS}" style="width:100%;height:100vh;border:none;"></iframe>
      </body>
    </html>
  `);
});
